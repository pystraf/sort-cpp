...
namespace sortcpp {
    namespace _internal {
        ...
        namespace american_flag_sorting {
            template<class RandomAccessIterator, class Key>
            int max_number_of_digits(RandomAccessIterator first, RandomAccessIterator last, int buckets, Key key) {
                int max = 0;
                for (auto i = first; i < last; i++) {
                    auto val = key(*i);
            ...
            template<class T>
            T _get_digit(T integer, int divisor, int buckets) {
                return (integer / divisor) % buckets;
            }

            template<class RandomAccessIterator, class Key>
            void _american_flag_sort(RandomAccessIterator start, RandomAccessIterator end, int divisor, int buckets, Key key) {
                std::vector<int> count(buckets, 0);
                std::vector<RandomAccessIterator> offset(buckets);

                for (auto i = start; i < end; i++) {
                    auto d = key(*i);
                    auto digit = _get_digit(d, divisor, buckets);
                    count[digit]++;
                }
                ...
                for (int b = 0; b < buckets; b++) {
                    while (count[b] > 0) {
                        ...
                        do {
                            auto digit = _get_digit(key(num), divisor, buckets);
                ...
                if (divisor > 1) {
                    for (int i = 0; i < buckets; i++) {
                        ...
                        auto ed = offset[i];
                        if (ed - bg > 1) _american_flag_sort(bg, ed, divisor / buckets, buckets, key);
                    }
                }
            }

            template<class RandomAccessIterator, class Key>
            void american_flag_sort(RandomAccessIterator first, RandomAccessIterator last, int buckets, Key key) {
                int number_of_digits = max_number_of_digits(first, last, buckets, key);
                int max = 1;
                for (int i = 0; i < number_of_digits - 1; i++) max *= buckets;
                _american_flag_sort(first, last, max, buckets, key);
            }
        }

        namespace counting_sorting {
            template<class RandomAccessIterator, class Key>
        ...
        namespace flash_sorting {
            using sortcpp::_internal::insertion_sorting::insertion_sort;
            template<class T, class RandomAccessIterator, class Key, class Compare>
            void _flash_sort(RandomAccessIterator first, RandomAccessIterator last, Key key, Compare comp) {
                ...
                for (K = m - 1; K >= 1; K--) {
                    ...
                    if (class_size > threshold && class_size > min_elements)
                        _flash_sort<T>(first + L[K], first + L[K + 1], key, comp);
                }
                insertion_sort(first, last, comp);
            }
            ...
            struct key_to_cmp {
                Key key;
                key_to_cmp(Key&& key) : key(std::forward<Key>(key)) {}
            ...
            void flash_sort(RandomAccessIterator first, RandomAccessIterator last, Key key) {
                ...
                key_to_cmp<T, Key> comp(std::move(key));
                _flash_sort<T>(first, last, key, comp);
            }
        }

        namespace radix_sorting {
            using sortcpp::_internal::get_digit;
            using sortcpp::_internal::fancy_transcribe;
            using sortcpp::_internal::transcribe_msd;

            template<class RandomAccessIterator, class Key>
            void inplace_lsd_radix_sort(RandomAccessIterator first, RandomAccessIterator last, int base, Key key) {
                auto pos = first;
                int length = last - first;
            ...
            template<class RandomAccessIterator, class Key>
            void lsd_radix_sort(RandomAccessIterator first, RandomAccessIterator last, int base, Key key) {
                using T = typename std::iterator_traits<RandomAccessIterator>::value_type;
            ...
            template<class T, class RandomAccessIterator, class Key>
            void radix_msd(RandomAccessIterator min, RandomAccessIterator max, int radix, int pow, Key key) {
                ...
                for (int i = 0; i < registers.size(); i++) {
                    radix_msd<T>(min + sum, min + sum + registers[i].size(), radix, pow - 1, key);
                    sum += registers[i].size();
                }
            }

            template<class RandomAccessIterator, class Key>
            void msd_radix_sort(RandomAccessIterator first, RandomAccessIterator last, int base, Key key) {
                using T = typename std::iterator_traits<RandomAccessIterator>::value_type;
                ...
                radix_msd<T>(first, last, base, maxpower, key);
            }

            template<class RandomAccessIterator>
            void _multi_swap(RandomAccessIterator a, RandomAccessIterator b, int len) {
                for (int i = 0; i < len; i++) std::iter_swap(a + i, b + i);
            }
            ...
            template<class RandomAccessIterator, class T, class Key>
            RandomAccessIterator _bin_search(RandomAccessIterator a, RandomAccessIterator b, const T& d, int p, int base, Key key) {
                while (a < b) {
                    auto m = a + (b - a) / 2;
            ...
            template<class RandomAccessIterator, class Key>
            void _merge(RandomAccessIterator a, RandomAccessIterator m, RandomAccessIterator b, 
                        int da, int db, int p, int base, Key key) {
                if (b - a < 2 || db - da < 2) return;

                int dm = da + (db - da) / 2;
                auto m1 = _bin_search(a, m, dm, p, base, key);
                auto m2 = _bin_search(m, b, dm, p, base, key);

                _rotate(m1, m, m2);
                m = m1 + (m2 - m);

                _merge(m, m2, b, dm, db, p, base, key);
                _merge(a, m1, m, da, dm, p, base, key);
            }

            template<class RandomAccessIterator, class Key>
            void _merge_sort(RandomAccessIterator a, RandomAccessIterator b, int p, int base, Key key) {
                if (b - a < 2) return;

                auto m = a + (b - a) / 2;

                _merge_sort(a, m, p, base, key);
                _merge_sort(m, b, p, base, key);
                _merge(a, m, b, 0, base, p, base, key);
            }

            template<class RandomAccessIterator, class Key>
            void rotate_lsd_radix_sort(RandomAccessIterator first, RandomAccessIterator last, int base, Key key) {
                auto max = key(*first);
                for (auto i = first + 1; i < last; i++)
                    if (key(*i) > max) max = key(*i);
                int maxpower = (int)(std::log(max) / std::log(base));

                for (int p = 0; p <= maxpower; p++) _merge_sort(first, last, p, base, key);
            }

            template<class T>
            T shift(T n, int q, int base) {
                while (q > 0) {
                    n /= base;
                    q--;
                }
            ...
            template<class RandomAccessIterator, class Key>
            RandomAccessIterator _dist(RandomAccessIterator a, RandomAccessIterator b, int p, int base, Key key) {
                _merge_sort(a, b, p, base, key);
                return _bin_search(a, b, 1, p, base, key);
            }

            template<class RandomAccessIterator, class Key>
            void rotate_msd_radix_sort(RandomAccessIterator first, RandomAccessIterator last, int base, Key key) {
                auto max = key(*first);
                for (auto i = first + 1; i < last; i++)
                    if (key(*i) > max) max = key(*i);
                ...
                while (i < last) {
                    auto p = (b - i < 1) ? i : _dist(i, b, q, base, key);
                    ...
                    if (q == 0) {
                        ...
                        i = b;
                        while (b < last && shift(key(*b), q + 1, base) == shift(m, q + 1, base)) b++;
                    }
                    else {
                        b = p;
                        q--;
        ...
        namespace stackless_american_flag_sorting {
            using sortcpp::_internal::radix_sorting::shift;
            using sortcpp::_internal::get_digit;

            template<class RandomAccessIterator, class Key>
            RandomAccessIterator dist(RandomAccessIterator a, RandomAccessIterator b, int q, int r, 
                                      std::vector<int>& cnts, std::vector<int>& offs, Key key) {
                for (int i = 1; i < r; i++) {
                    cnts[i] += cnts[i - 1];
                    offs[i] = cnts[i - 1];
                }
                ...
                for (int i = 0; i < r - 1; i++) {
                    ...
                    if (cnts[i] > offs[i]) {
                        ...
                        do {
                            auto digit = get_digit(key(t), q, r);
                            cnts[digit]--;
            ...
            void american_flag_sort(RandomAccessIterator first, RandomAccessIterator last, int buckets, Key key) {
                ...
                for (auto j = i; j < b; j++) {
                    auto digit = get_digit(key(*j), q, r);
                    cnts[digit]++;
                }

                while (i < last) {
                    auto p = (b - i < 1) ? i : dist(i, b, q, r, cnts, offs, key);
                    ...
                    if (q == 0) {
                        ...
                        while (b < last && shift(key(*b), q + 1, r) == shift(m, q + 1, r)) {
                            auto digit = get_digit(key(*b), q, r);
                            cnts[digit]++;
                            b++;
                        }
                    }
        ...
        namespace stackless_binary_quick_sorting {
            using sortcpp::_internal::get_bit;
            using sortcpp::_internal::binary_quick_sorting::get_max_bit;

            template<class RandomAccessIterator, class Key>
            RandomAccessIterator binary_partition(RandomAccessIterator a, RandomAccessIterator b, int bit, Key key) {
                auto i = a - 1, j = b;
            ...
            template<class RandomAccessIterator, class Key>
            void binary_quick_sort(RandomAccessIterator first, RandomAccessIterator last, Key key) {
                auto i = first, b = last;
                int q = get_max_bit(first, last, key) - 1, m = 0;

                while (i < last) {
                    auto p = (b - i < 1) ? i : binary_partition(i, b, q, key);
        ...
        namespace static_sorting {
            using sortcpp::_internal::insertion_sorting::insertion_sort;
            using sortcpp::_internal::heap_sorting::heap_sort;
            using sortcpp::_internal::flash_sorting::key_to_cmp;

            template<class RandomAccessIterator, class Key, class Compare>
            void _static_sort(RandomAccessIterator a, RandomAccessIterator b, Key key, Compare comp) {
                auto min = *a, max = *b;
                for (auto i = a + 1; i < b; i++) {
                    if (key(*i) < min) min = *i;
                    if (key(*i) > max) max = *i;
                ...
                for (auto i = 0; i < aux_len; i++) {
                    ...
                    if (e - s <= 1) continue;
                    if (e - s > 16) heap_sort(s, e, true, comp);
                    else insertion_sort(s, e, comp);
                }
            }
            ...
            void static_sort(RandomAccessIterator first, RandomAccessIterator last, Key key) {
                ...
                key_to_cmp<T, Key> comp(std::move(key));
                _static_sort(first, last, key, comp);
            }
        }
    }
}
